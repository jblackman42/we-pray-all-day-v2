...
router.get('/getReservations', async (req, res) => {
  ...
  try {
    const data = await axios({
      ...
      params: {
        $filter: `WPAD_Community_Reservations.[Reservation_Date] BETWEEN '${startDate}' AND '${endDate}'`,
        $orderby: `WPAD_Community_Reservations.[Reservation_Date]`,
        $select: `WPAD_Community_Reservation_ID, WPAD_Community_Reservations.[Reservation_Date], WPAD_Community_Reservations.[WPAD_Community_ID], WPAD_Community_ID_Table.[Community_Name]`
      },
      headers: {
        'Authorization': `Bearer ${await getAccessToken()}`,
        'Content-Type': 'Application/Json'
