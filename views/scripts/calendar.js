class PrayerCalendar extends HTMLElement {
  ...
  update = async (year, month) => {
    ...
    daysGrid.innerHTML = this.monthDays.map(data => {
      ...
      return `
        <button ${!blocked ? `onclick="window.location='/signup?date=${data.date}'"` : ''} class="calendar-day ${!blocked ? 'pointer' : ''} ${currChampionsIDs.includes(currCommunityFilter) || currCommunityFilter == 0 ? '' : 'dull'}">
